...
class TransferTool:
    def __init__(self, config: ConfigParser):
        if (
            not config.has_option("transfer", "source_site")
            or not config.has_option("transfer", "destination_site")
            or not config.has_option("transfer", "destination_username")
        ):
        ...
        self.destination_wiki_site = pywikibot.Site(
            code=self.destination_site,
            fam=family,
        ...
        if not self.destination_wiki_site.logged_in():
            ...
            if not self.destination_wiki_site.logged_in():
                raise RuntimeError(
                    "Login with pywikibot failed to destination site failed."
                )
        # Set throttle to 0 to speed up write operations (otherwise pywikibot would wait up to 10s after each write)
        self.destination_wiki_site.throttle.set_delays(
            delay=0, writedelay=0, absolute=True
        )

        self.source_fortraininglib: ForTrainingLib = ForTrainingLib(
            family.base_url(self.source_site, ""), family.scriptpath(self.source_site)
        )
        self.destination_fortraininglib: ForTrainingLib = ForTrainingLib(
            family.base_url(self.destination_site, ""),
            family.scriptpath(self.destination_site),
        )

    def transfer(self, page_name, language_code, message=None):
        source_translation_page: Optional[TranslatedPage] = (
            self.source_fortraininglib.get_translation_units(page_name, language_code)
        )
        ...
        for source_translation_unit in source_translation_page:
            ...
            self.upload(
                f"{source_translation_unit.identifier}/{language_code}",
                source_translation,
                message,
            )
        ...
        print(
            f"Transfer of {numTotal} elements for '{page_name}/{language_code}' "
            + f"from '{self.source_site}' to '{self.destination_site}' completed."
        )
        print(
            f"unchanged: {self.unchanged} | modified: {self.modified} | created: {self.created}"
        )
    ...
    def upload(self, identifier: str, translated_text: str, message=None):
        ...
        destination_mediawiki_page = pywikibot.Page(
            self.destination_wiki_site, f"Translations:{identifier}"
        )
