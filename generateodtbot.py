...
class GenerateODTBot:
    def __init__(self, config: ConfigParser):
        ...
        if log_path == "":
            self.logger.warning(
                "No log directory specified in configuration. Using current working directory"
            )
        ...
        if config.has_option("generateodtbot", "debuglogfile"):
            fh_debug = logging.FileHandler(
                f"{log_path}{config['generateodtbot']['debuglogfile']}"
            )
    ...
    def notify_user(
        self, username: str, worksheet: str, languagecode: str, admin: bool
    ):
...
"""
            ...
            self.logger.error(
                f"User {username} is not emailable. No notification sent."
            )

    def run(self, worksheet: str, languagecode: str, username: str):
        self.logger.debug(
            f"worksheet: {worksheet}, languagecode: {languagecode}, username: {username}"
        )
        ...
        if base_path == "":
            self.logger.warning(
                "No base directory specified in configuration. Using current working directory"
            )
        ...
        while not got_lock:
            try:
                ...
                retries += 1
                self.logger.debug(
                    f"Couldn't get exclusive lock. Waiting 5s and trying again. Attempt #{retries}"
                )
                if retries > CONNECT_TRIES:
                    self.logger.error(
                        f"Couldn't get exclusive lock. Tried {CONNECT_TRIES} times, giving up."
                    )
                    sys.exit(1)
                time.sleep(5)
            else:
        ...
        try:
            ...
            if isinstance(filename, str):
                ...
                if not dropboxupload.upload_file(
                    languagecode, filename.replace(".odt", ".pdf")
                ):
                    self.logger.error(
                        f"Dropbox upload of {filename.replace('odt', '.pdf')} failed!"
                    )
            else:
                self.logger.error(
                    "Translateodt failed. See log above or ask an administrator for help."
                )
            if not dropboxupload.upload_string(
                languagecode, f"log/{worksheet}.txt", self.stream.getvalue()
            ):
                self.logger.error(f"Dropbox upload of log/{worksheet}.txt failed")
            if not dropboxupload.upload_string(
                languagecode, f"log/{worksheet}.debug.txt", self.stream_debug.getvalue()
            ):
                self.logger.error(f"Dropbox upload of log/{worksheet}.debug.txt failed")

            if not config.has_option("generateodtbot", "site") or not config.has_option(
                "generateodtbot", "username"
            ):
                self.logger.error(
                    "Missing connection settings for generateodtbot in config.ini. "
            ...
            self.site = pywikibot.Site(
                code=code, fam=family, user=config.get("generateodtbot", "username")
            )
            ...
            if self.site.logged_in():
                ...
                if config.has_option("generateodtbot", "admin"):
                    self.notify_user(
                        config["generateodtbot"]["admin"], worksheet, languagecode, True
                    )
                else:
                    self.logger.warning(
                        "No admin in configuration defined. Sending no admin notification"
                    )
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "username", help="username to send notification after we're finished"
    )
    parser.add_argument(
        "-l",
        "--loglevel",
