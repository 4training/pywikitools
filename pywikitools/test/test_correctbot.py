...
def title_correct(
    corrector: CorrectorBase, text: str, original: Optional[str] = None
) -> str:
...
class CorrectorTestCase(unittest.TestCase):
    ...
    def compare_revisions(
        ...
        old_content = fortraininglib.get_translated_unit(
            page, language_code, identifier, old_revision
        )
        new_content = fortraininglib.get_translated_unit(
            page, language_code, identifier, new_revision
        )
        ...
        self.assertEqual(
            correct(self.corrector, old_content, original_content), new_content
        )

    def compare_title_revisions(
        self, page: str, language_code: str, old_revision: int, new_revision
    ):
        """Calls CorrectBase.title_correct()"""
        fortraininglib = ForTrainingLib("https://test.4training.net")
        old_content = fortraininglib.get_translated_title(
            page, language_code, old_revision
        )
        new_content = fortraininglib.get_translated_title(
            page, language_code, new_revision
        )
        fortraininglib.session.close()
        self.assertIsNotNone(old_content)
        self.assertIsNotNone(new_content)
    ...
    def test_for_meaningful_names(self):
        ...
        for language_corrector in self.language_correctors:
            for function_name in dir(language_corrector):
                ...
                )  # Special case from CorrectBot.check_unit()
                self.assertTrue(
                    function_name.startswith("correct_")
        ...
        for language_corrector in self.language_correctors:
            for function_name in dir(language_corrector):
                ...
                self.assertGreaterEqual(
                    count_parameters,
                    1,
                    msg=f"{language_corrector.__name__}.{function_name}",
                )
        ...
        for language_corrector in self.language_correctors:
            for language_function in dir(language_corrector):
                if language_function.startswith("_"):
                    continue
                if (
                    getattr(language_corrector, language_function).__module__
                    != MOD_UNIVERSAL
                ):
                    self.assertNotIn(
                        language_function,
                        flexible_functions,
                        msg=f"{language_corrector.__name__}: "
    ...
    def test_for_function_documentation(self):
        ...
        for language_corrector in self.language_correctors:
            for function_name in dir(language_corrector):
                ...
                self.assertTrue(
                    corrector_function.__doc__.strip(),
                ...
                self.assertTrue(
                    corrector_function.__doc__.partition("\n")[0].strip(),
...
class TestUniversalCorrector(unittest.TestCase):
    ...
    def test_spaces(self):
        self.assertEqual(
            correct(self.corrector, "This entry   contains     too  many spaces."),
            "This entry contains too many spaces.",
        )
        self.assertEqual(
            correct(self.corrector, "Missing.Spaces,after punctuation?Behold,again."),
        ...
        self.assertEqual(
            correct(self.corrector, "Before  and   after."), "Before and after."
        )
        # now let's try if exceptions are correctly respected:
        self.assertEqual(
            correct(self.corrector, "John 3:16.Johannes 3,16."),
            "John 3:16. Johannes 3,16.",
        )
        self.assertEqual(
            correct(self.corrector, "Test.This remains untouched."),
            "Test.This remains untouched.",
        )
        self.assertEqual(
            correct(self.corrector, "Do not touch Test.This"), "Do not touch Test.This"
        )
        self.assertEqual(
            correct(self.corrector, "Use e.g. unittest"), "Use e.g. unittest"
        )
        ...
        self.assertEqual(
            correct(self.corrector, "How do I survive if ... ?"),
            "How do I survive if ... ?",
        )
        self.assertEqual(
            correct(self.corrector, "How do I survive if … ?"),
        ...
        self.assertEqual(
            correct(self.corrector, "□ Yes    □ No    □"), "□ Yes    □ No    □"
        )

    def test_print_stats(self):
        self.assertIn(
            "Correct mediawiki links", self.corrector.print_stats({"correct_links": 4})
        )
        stats: Dict[str, int] = {
            "correct_multiple_spaces_also_in_title": 2,
        ...
        self.assertIn(
            "Correct filename", self.corrector.print_stats({"filename_correct": 5})
        )
    ...
    def test_capitalization(self):
        ...
        self.assertEqual(
            correct(self.corrector, "After colons: and semicolons; we don't correct."),
            "After colons: and semicolons; we don't correct.",
        )

    def test_dash_correction(self):
        self.assertEqual(
            correct(self.corrector, "Using long dash - not easy."),
            "Using long dash – not easy.",
        )
    ...
    def test_final_dot_correction(self):
        ...
        self.assertEqual(
            correct(self.corrector, "Ein ganzer Satz", "A full sentence"),
            "Ein ganzer Satz",
        )

    def test_mediawiki_bold_italic(self):
        self.assertEqual(
            correct(self.corrector, "''italic'' and '''bold'''"),
            "<i>italic</i> and <b>bold</b>",
        )
        self.assertEqual(
            correct(self.corrector, "This is '''''italic and bold'''''."),
            "This is <b><i>italic and bold</i></b>.",
        )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ):
            self.assertEqual(
                correct(self.corrector, "'''''Uneven markup"), "'''''Uneven markup"
            )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ):
            self.assertEqual(
                correct(self.corrector, "''''Problematic markup"),
                "''''Problematic markup",
            )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
            ...
            self.assertEqual(
                correct(self.corrector, "'''Uneven''' '''markup"),
                "'''Uneven''' '''markup",
            )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ...
        with self.assertLogs(
            ...
            self.assertEqual(
                correct(self.corrector, "<b>Uneven tags<b>"), "<b>Uneven tags<b>"
            )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ):
            self.assertEqual(
                correct(self.corrector, "</i>Uneven tags</i>"), "</i>Uneven tags</i>"
            )
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ):
            self.assertEqual(
                correct(self.corrector, ">i<Confused tags>b<"), "<i>Confused tags<b>"
            )
    ...
    def test_correct_links(self):
        ...
        self.assertEqual(
            self.corrector.correct_links("[[Dest|Desc]]", "[[Orig|Orig]]"),
            "[[Dest|Desc]]",
        )
        ...
        with self.assertLogs(
            ...
            self.assertEqual(
                self.corrector.correct_links("[[Simple link]]", "[[Orig]]"),
                "[[Simple link]]",
            )
    ...
    def test_remove_trailing_dot_in_title(self):
        ...
        self.assertEqual(
            title_correct(self.corrector, "Title with no dot"), "Title with no dot"
        )
...
class NoSpaceBeforePunctuationCorrectorTester(
    CorrectorBase, NoSpaceBeforePunctuationCorrector
):
...
class TestNoSpaceBeforePunctuationCorrector(CorrectorTestCase):
    def test_spaces(self):
        ...
        self.assertEqual(
            correct(corrector, "I want this ! And you   ?"), "I want this! And you?"
        )
        self.assertEqual(
            correct(corrector, "I want ... ! How about ___ ?"),
            "I want ... ! How about ___ ?",
        )
        self.assertEqual(
            correct(corrector, "Ellipsis … ? Sure … !"), "Ellipsis … ? Sure … !"
        )
...
class TestQuotationMarkCorrector(CorrectorTestCase):
    def test_correct_quotes(self):
        ...
        for wrong in [
            ...
            self.assertEqual(
                correct(corrector, f"Start and {wrong}"), "Start and »Test«"
            )
            self.assertEqual(correct(corrector, f"{wrong} and end."), "»Test« and end.")
            self.assertEqual(
                correct(corrector, f"Start and {wrong} and end."),
                "Start and »Test« and end.",
            )
            self.assertEqual(
                correct(corrector, f"{wrong}, {wrong} and “Test”."),
                "»Test«, »Test« and »Test«.",
            )
...
class TestRTLCorrector(CorrectorTestCase):
    ...
    def test_correct_punctuation(self):
        ...
        self.assertEqual(
            correct(self.corrector, "ما هو من عند الله?"), "ما هو من عند الله؟"
        )
        # Make sure mediawiki : and ; formatting (at beginning of lines) remain unchanged
        self.assertEqual(
            correct(self.corrector, ";التدوين و الكتابة\n:من"),
            ";التدوين و الكتابة\n:من",
        )

    def test_fix_rtl_title(self):
        self.compare_title_revisions(
            "Bible_Reading_Hints_(Seven_Stories_full_of_Hope)", "fa", 57796, 62364
        )
...
class TestGermanCorrector(CorrectorTestCase):
    ...
    def test_correct_quotes(self):
        for wrong in [
            ...
            self.assertEqual(
                correct(self.corrector, f"Beginn und {wrong}"), "Beginn und „Test“"
            )
            self.assertEqual(
                correct(self.corrector, f"{wrong} und Ende."), "„Test“ und Ende."
            )
            self.assertEqual(
                correct(self.corrector, f"Beginn und {wrong} und Ende."),
                "Beginn und „Test“ und Ende.",
            )
        ...
        for valid in valid_strings:
            ...
            self.assertEqual(
                correct(self.corrector, valid), valid
            )  # Check that correct strings remain correct
    ...
    def test_correct_links(self):
        ...
        english = (
            "See [[Prayer|Prayer]] and [[Healing|Healing]][[Test|Test]] or [[#below]]."
        )
        self.assertEqual(correct(self.corrector, to_correct, english), corrected)
        self.assertEqual(
            correct(self.corrector, corrected, english), corrected
        )  # correct text should not change
        with self.assertLogs(
            "pywikitools.correctbot.correctors.universal", level="WARNING"
        ):
            # Warn because the number of links in translation is less than in the English original
            self.assertEqual(
                correct(self.corrector, to_correct[21:], english), to_correct[21:]
            )
...
FRENCH_CORRECTIONS: Dict[str, str] = {
    "Mais moi , je vous dis: Si": "Mais moi, je vous dis\u00a0: Si",
    "Si quelqu’un dit à son frère: Imbécile!": "Si quelqu’un dit à son frère\u00a0: Imbécile\u00a0!",
    "Comment prier?<br/>Comment jeûner?": "Comment prier\u00a0?<br/>Comment jeûner\u00a0?",
    "Ne jugez pas les autres,et Dieu ne vous jugera pas . En effet ,Dieu": "Ne jugez pas les autres, et Dieu ne vous jugera pas. En effet, Dieu",
}
...
class TestFrenchCorrector(unittest.TestCase):
    ...
    def test_correct_quotes(self):
        ...
        self.assertEqual(
            correct(corrector, " Connect ".join(wrongs)),
            " Connect ".join(["«\u00a0Test\u00a0»"] * 3),
        )
...
class TestTurkishCorrector(CorrectorTestCase):
    def test_all_rules(self):
        ...
        self.assertEqual(
            corrector._compose_filename("Kutsal_Kitabı_okuma_ipuçları", ".pdf", True),
            "Kutsal_Kitabı_okuma_ipuçları_yazdır.pdf",
        )
        self.assertEqual(
            correct(
                corrector, '"küçük"  bir günah işlediler - bizim için.devam , bitti .'
            ),
            "“küçük” bir günah işlediler – bizim için. Devam, bitti.",
        )
        self.assertEqual(
            correct(corrector, "1.Yuhanna, 1.Petrus,1.Timoteos vs. okuyun ."),
...
class TestTurkishSecularCorrector(CorrectorTestCase):
    def test_all_rules(self):
        ...
        self.assertEqual(
            corrector._compose_filename("Şifa", ".pdf", False), "Şifa_(Tanrı).pdf"
        )
        self.assertEqual(
            corrector._compose_filename("Kutsal_Kitabı_okuma_ipuçları", ".pdf", True),
        ...
        self.assertEqual(
            corrector._compose_filename("Tanrının_Hikâyesi", ".pdf", False),
...
class TestArabicCorrector(CorrectorTestCase):
    ...
    def test_real_life_examples(self):
        self.compare_revisions(
            "How to Continue After a Prayer Time", "ar", 4, 62201, 62260
        )
        self.compare_revisions(
            "How to Continue After a Prayer Time", "ar", 16, 62225, 62270
        )
        self.compare_title_revisions(
            "How to Continue After a Prayer Time", "ar", 62193, 62274
        )
        self.compare_revisions(
            "How_to_Continue_After_a_Prayer_Time", "ar", 1, 62195, 62258
        )
...
class TestPersianCorrector(CorrectorTestCase):
    def test_correct_quotes(self):
        ...
        self.assertEqual(
            correct(corrector, 'کلمه "شنیدن" استفاده'), "کلمه «شنیدن» استفاده"
        )
        self.assertEqual(
            correct(corrector, "کلمه ”شنیدن“ استفاده"), "کلمه «شنیدن» استفاده"
        )
...
class TestCorrectBot(unittest.TestCase):
    def setUp(self):
        ...
        self.config.read_dict(
            {"correctbot": {"site": "test", "username": "TestCorrectBot"}}
        )
    ...
    def test_check_unit(self):
        ...
        self.assertIsNone(
            self.correctbot.check_unit(
                corrector, TranslationUnit("Test/1", "fr", "Test", "")
            )
        )
        ...
        self.assertIsNone(
            self.correctbot.check_unit(
                corrector, TranslationUnit("Test/1", "fr", "1.2", "1.2a")
            )
        )
        ...
        for faulty, corrected in FRENCH_CORRECTIONS.items():
            ...
            result = self.correctbot.check_unit(
                corrector, translation_unit, "not_existing_rule"
            )
            self.assertEqual(faulty, result.corrections.get_original_translation())
        ...
        translation_unit = TranslationUnit(
            "Test", "fr", "ignored", "Mais moi , je vous dis: Si"
        )
        result = self.correctbot.check_unit(
            corrector, translation_unit, "correct_spaces_before_comma_and_dot"
        )
        self.assertEqual(
            "Mais moi, je vous dis: Si", result.suggestions.get_translation()
        )
        result = self.correctbot.check_unit(
            corrector, translation_unit, "correct_spaces_before_punctuation"
        )
        self.assertEqual(
            "Mais moi , je vous dis\u00a0: Si", result.suggestions.get_translation()
        )
        ...
        unit_title = TranslationUnit(
            "Test/Page_display_title", "fr", "Testing: Now", "Tester: Maintenant"
        )
        ...
        self.assertFalse(
            unit_filename.has_translation_changes()
        )  # The unit we gave shouldn't be touched
        # A special PDF for printing should receive the translated suffix
        unit_filename_print = TranslationUnit(
            "Test/6", "fr", "Testing_Now_print.pdf", "Pas_correcte.pdf"
        )
        result = self.correctbot.check_unit(corrector, unit_filename_print)
        self.assertEqual(
            result.corrections.get_translation(), "Tester_Maintenant_impression.pdf"
        )
        ...
        unit = TranslationUnit(
            "Test/2", "fr", "Please translate this.", "Please translate this."
        )
    ...
    def prepare_translated_page(self) -> TranslatedPage:
        ...
        for counter, faulty in enumerate(FRENCH_CORRECTIONS.keys(), start=1):
            ...
            translation_units.append(
                TranslationUnit(f"Test/{counter}", "fr", original, faulty)
            )
        ...
        translation_units.append(
            TranslationUnit(
                "Test/warnings2", "fr", 'this is a "quote"', 'Ceci est une „citation"'
            )
        )
    ...
    def test_check_page(self):
        ...
        for result in results:
            ...
            self.assertEqual(
                FRENCH_CORRECTIONS[faulty], result.suggestions.get_translation()
            )
    ...
    def test_save_report(self, mock_page):
        ...
        with open(
            join(dirname(abspath(__file__)), "data", "correctbot_report.mediawiki"), "r"
        ) as f:
