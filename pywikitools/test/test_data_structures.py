...
TEST_URL: str = (
    "https://www.4training.net/mediawiki/images/7/70/Gottes_Reden_wahrnehmen.pdf"
)
# a different url
TEST_URL2: str = (
    "https://www.4training.net/mediawiki/images/1/15/Gottes_Reden_wahrnehmen.pdf"
)
...
class TestTranslationProgress(unittest.TestCase):
    def test_everything(self):
        ...
        for counter, progress_dict in enumerate(
            [TEST_PROGRESS, TEST_PROGRESS2, TEST_PROGRESS3]
        ):
...
class TestFileInfo(unittest.TestCase):
    ...
    def test_serialization(self):
        ...
        file_info = FileInfo(
            "pdf",
            TEST_URL,
            datetime.fromisoformat(TEST_TIME),
            translation_unit=5,
            metadata=summary,
        )
        json_text = DataStructureEncoder().encode(file_info)
...
class TestWorksheetInfo(unittest.TestCase):
    def setUp(self):
        ...
        self.worksheet_info = WorksheetInfo(
            TEST_EN_NAME, TEST_LANG, TEST_TITLE, self.progress, TEST_VERSION
        )
    ...
    def test_add_file_info(self):
        ...
        self.assertEqual(
            self.worksheet_info.get_file_type_name("pdf"), "Gottes_Reden_wahrnehmen.pdf"
        )

        # add_file_info() should accept "2018-12-20T12:58:57Z" as well
        test_time = TEST_TIME.replace("+00:00", "Z")
        self.worksheet_info.add_file_info(
            FileInfo("odt", TEST_URL.replace(".pdf", ".odt"), test_time)
        )
    ...
    def test_add_file_info_errors(self):
        with self.assertLogs("pywikitools.resourcesbot.fileinfo", level="ERROR"):
            self.worksheet_info.add_file_info(
                FileInfo("odg", TEST_URL, "2018-12-20-12-58-57")
            )
        # TODO add tests for call with from_pywikibot= (pywikibot.page.FileInfo)
    ...
    def test_get_file_infos(self):
        ...
        self.assertEqual(
            list(self.worksheet_info.get_file_infos().keys()), expected_file_types
        )
    ...
    def test_show_in_list(self):
        english_info = WorksheetInfo(
            TEST_EN_NAME, "en", TEST_EN_NAME, self.progress, TEST_VERSION
        )
    ...
    def test_serialization(self):
        ...
        worksheet_info = WorksheetInfo(
            "Prayer", TEST_LANG, "Gebet", progress, "TEST_VERSION"
        )
        ...
        self.assertEqual(
            DataStructureEncoder().encode(decoded_worksheet_info), json_text
        )
        ...
        worksheet_info.add_file_info(
            FileInfo("odt", TEST_URL.replace(".pdf", ".odt"), TEST_TIME)
        )
        worksheet_info.add_file_info(
            FileInfo("printPdf", TEST_URL.replace(".pdf", "_print.pdf"), TEST_TIME)
        )
        json_text = DataStructureEncoder().encode(worksheet_info)
        decoded_worksheet_info = json.loads(json_text, object_hook=json_decode)
        self.assertIsInstance(decoded_worksheet_info, WorksheetInfo)
    ...
    def test_to_str(self):
        ...
        progress = TranslationProgress(**TEST_PROGRESS)
        with_unit = WorksheetInfo(
            TEST_EN_NAME, TEST_LANG, TEST_TITLE, progress, TEST_VERSION, "2"
        )
    ...
    def test_has_same_version(self):
        ...
        self.assertTrue(self.worksheet_info.has_same_version(english_info))
        self.assertTrue(
            self.worksheet_info.has_same_version(
                english_info, check_only_major_version=True
            )
        )
        ...
        tamil_info = WorksheetInfo(
            TEST_EN_NAME, "ta", "கர்த்தரிடமிருந்து கேட்பது", self.progress, "௧.௨"
        )
        ...
        self.assertTrue(
            tamil_info.has_same_version(english_info, check_only_major_version=True)
        )
        kannada_info = WorksheetInfo(
            TEST_EN_NAME, "kn", "ದೇವರಿಂದ ಕೇಳುವುದು", self.progress, "೧.೨"
        )
        ...
        hindi_info = WorksheetInfo(
            TEST_EN_NAME, "hi", "परमेश्वर के साथ का समय", self.progress, "१.२"
        )
        ...
        self.assertTrue(
            hindi_info.has_same_version(english_info, check_only_major_version=True)
        )
...
class TestLanguageInfo(unittest.TestCase):
    ...
    def test_basic_functionality(self):
        progress = TranslationProgress(**TEST_PROGRESS)
        worksheet_info = WorksheetInfo(
            TEST_EN_NAME, TEST_LANG, TEST_TITLE, progress, TEST_VERSION
        )
    ...
    def test_worksheet_has_type(self):
        ...
        self.language_info.get_worksheet(TEST_EN_NAME).add_file_info(
            FileInfo("pdf", TEST_URL, TEST_TIME)
        )
    ...
    def test_serialization(self):
        ...
        self.language_info.get_worksheet(TEST_EN_NAME).add_file_info(
            FileInfo("odt", TEST_URL2, TEST_TIME)
        )
        progress = TranslationProgress(**TEST_PROGRESS)
        worksheet_info = WorksheetInfo(
            "Prayer", TEST_LANG, "Gebet", progress, TEST_VERSION
        ...
        self.assertEqual(
            DataStructureEncoder().encode(decoded_language_info), json_text
        )
    ...
    def test_basic_comparison(self):
        ...
        old_language_info = json.loads(
            DataStructureEncoder().encode(self.language_info), object_hook=json_decode
        )
        self.assertTrue(self.language_info.compare(old_language_info).is_empty())

        # Add an ODT file
        self.language_info.worksheets[TEST_EN_NAME].add_file_info(
            FileInfo("odt", TEST_URL2, TEST_TIME)
        )
...
class TestLanguageInfoComparison(unittest.TestCase):
    ...
    def test_added_files(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_added_files.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
            [change_item for change_item in language_info2.compare(self.language_info)]
        )
        self.assertSetEqual(
            changes,
            set(
                [
                    ChangeItem("Church", ChangeType.NEW_PDF),
    ...
    def test_deleted_files(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_deleted_files.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    ChangeItem("Church", ChangeType.DELETED_ODT),
    ...
    def test_deleted_worksheet(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_deleted_worksheet.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    ChangeItem("Prayer", ChangeType.DELETED_WORKSHEET),
    ...
    def test_new_worksheet(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_new_worksheet.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    ChangeItem(
                        "Confessing_Sins_and_Repenting", ChangeType.NEW_WORKSHEET
                    ),
    ...
    def test_updated_files(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_updated_files.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    ChangeItem("Church", ChangeType.UPDATED_ODT),
    ...
    def test_updated_worksheet(self):
        with open(
            join(dirname(abspath(__file__)), "data", "ru_updated_worksheet.json"), "r"
        ) as f:
            language_info2 = json.load(f, object_hook=json_decode)
        self.assertIsInstance(language_info2, LanguageInfo)
        changes = set(
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    ChangeItem("Hearing_from_God", ChangeType.UPDATED_WORKSHEET),
