...
class TestTranslateODT(unittest.TestCase):
    ...
    def test_is_search_and_replace_necessary(self):
        is_necessary = (
            self.translate_odt._is_search_and_replace_necessary
        )  # Shorten this long name
        for extension in ForTrainingLib("").get_file_types():
            self.assertFalse(
                is_necessary(f"Test.{extension}", f"Translation.{extension}")
            )
    ...
    def test_get_odt_filename(self):
        ...
        headline = TranslationUnit(
            "Test/Page_display_title", "de", "Good Title", "Guter Titel"
        )
        ...
        self.assertEqual(
            self.translate_odt._get_odt_filename(translated_page), "Guter_Titel.odt"
        )
    ...
    def test_persian_properties(self):
        ...
        self.translate_odt._set_properties(translated_page)
        self.translate_odt._loffice.set_properties.assert_called_with(
            "شفا", "Healing Persian Farsi فارسی", ANY
        )
    ...
    def test_cleanup_units(self):
        ...
        unit1 = TranslationUnit(
            "Test/2", "de", "We're drowning in snow chaos", "Wir versinken im Schnee."
        )
        unit2 = TranslationUnit("Test/1", "de", "sin", "Sünde")
        translated_page = TranslatedPage("Test", "de", [unit1, unit2])
        with self.assertLogs("pywikitools.translateodt", level="WARNING"):
            cleaned_up_page = self.translate_odt._cleanup_units(
                translated_page, TranslateOdtConfig()
            )
        ...
        self.assertListEqual(
            [tu.identifier for tu in cleaned_up_page.units], ["Test/1"] * 3
        )

    def _sort_units(
        self, definitions: List[str], translations: List[str]
        ...
        for counter, definition in enumerate(definitions):
            unit = TranslationUnit(
                f"Test/{counter}", "de", definition, translations[counter]
            )
            units.append(unit)
    ...
    def test_special_sort_units(self):
        ...
        with self.assertLogs("pywikitools.translateodt", level="INFO"):
            (orig_list, trans_list) = self._sort_units(
                ["this", "this", "is", "thistle"], ["same", "same", "same", "same"]
            )
        ...
        with self.assertLogs("pywikitools.translateodt", level="INFO"):
            (orig_list, trans_list) = self._sort_units(
                ["this<br/>is", "this<br/>thistle"], ["same<br/>same", "same<br/>same"]
            )
        ...
        with self.assertLogs("pywikitools.lang.TranslationUnit", level="WARNING") as cm:
            (orig_list, trans_list) = self._sort_units(
                ["something<br/>range", "thing<br/>strange"],
    ...
    def test_read_worksheet_config(self):
        ...
        with open(
            join(dirname(abspath(__file__)), "data", "Bible_Reading_Hints.config"), "r"
        ) as f:
            test_config = f.read()
        ...
        self.assertSetEqual(
            ...
            set(
                [
                    "Bible_Reading_Hints/1",
                    "Bible_Reading_Hints/2",
                    "Bible_Reading_Hints/3",  # noqa: E128
                    "Template:BibleReadingHints/18",
                    "Template:BibleReadingHints/25",
                    "Template:BibleReadingHints/26",
                ]
            ),
        )
    ...
    def test_translate_worksheet_aborts(self, mock_correctbot, mock_fortraininglib):
        ...
        mock_fortraininglib.return_value.get_translation_units.return_value = (
            mock_translated_page
        )
        with self.assertLogs("pywikitools.translateodt", level="ERROR"):
            self.assertIsNone(translateodt.translate_worksheet("Prayer", "de"))
