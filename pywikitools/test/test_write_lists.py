...
class TestWriteList(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.config["resourcesbot"] = {"username": "", "password": ""}
        with self.assertLogs(
            "pywikitools.resourcesbot.modules.write_lists", level="WARNING"
        ):
            self.write_list = WriteList(
        ...
        # Create a pseudo English LanguageInfo - enough for our testing purposes
        ...
        with open(
            join(
                dirname(abspath(__file__)), "data", "Russian_resources_list.mediawiki"
            ),
            "r",
        ) as f:
    ...
    def test_create_file_mediawiki(self):
        pdf_mediawiki = (
            r" [[File:pdficon_small.png|link={{filepath:Gottes_Reden_wahrnehmen.pdf}}]]"
        )
        ...
        # Test a "normal" call
        self.assertEqual(
            self.write_list._create_file_mediawiki(file_info), pdf_mediawiki
        )
        # Test for robust handling if URL is just a filename
        file_info = FileInfo(
            "pdf", "Gottes_Reden_wahrnehmen.pdf", "2018-12-23T13:11:23+00:00"
        )
        with self.assertLogs(
            "pywikitools.resourcesbot.modules.write_lists", level="WARNING"
        ):
    ...
    def test_create_mediawiki(self):
        ...
        self.assertEqual(
            self.write_list.create_mediawiki(self.language_info, self.english_info),
            self.expected_output,
        )

    def test_find_resources_list(self):
        page_content1 = (
            "Some text\n== <translate>Available training resources in"
            " Turkish (secular)</translate> ==\n"
        )
        resources_list = "* List item 1\n* List item 2"
        page_content = page_content1 + resources_list
        # There is no list for German in page_content
        self.assertEqual(
            self.write_list._find_resources_list(page_content, "German"), (0, 0)
        )
        # There is no list in page_content1
        self.assertEqual(
            self.write_list._find_resources_list(page_content1, "Turkish (secular)"),
            (0, 0),
        )
        ...
        pos_start, pos_end = self.write_list._find_resources_list(
            page_content, "Turkish (secular)"
        )
        ...
        with self.assertLogs(
            "pywikitools.resourcesbot.modules.write_lists", level="INFO"
        ):
            page_with_two_lists = (
                page_content + "\n== Turkish (another variant) ==\n" + resources_list
            )
            pos_start2, pos_end2 = self.write_list._find_resources_list(
                page_with_two_lists, "Turkish (secular)"
            )
    ...
    def test_run_edge_cases(self, mock_page):
        ...
        # run() should warn and directly return if the language name is missing
        # in LanguageInfo
        problematic_language_info = LanguageInfo("de", "")
        changes.add_change(
            "Prayer", ChangeType.NEW_WORKSHEET
        )  # we need a relevant change
        with self.assertLogs(
            "pywikitools.resourcesbot.modules.write_lists", level="WARNING"
        ):
            self.write_list.run(
                problematic_language_info, self.english_info, changes, ChangeLog()
            )
        ...
        with self.assertLogs(
            ...
            self.write_list.run(
                not_existing_language_info, self.english_info, changes, ChangeLog()
            )
        mock_page.return_value.exists.assert_called_once()
        mock_page.return_value.isRedirectPage.assert_not_called()

        # run() should warn and return if language information page is redirect but the
        ...
        mock_page.return_value.getRedirectTarget.return_value.exists.return_value = (
            False
        )
        with self.assertLogs(
            "pywikitools.resourcesbot.modules.write_lists", level="WARNING"
        ...
        # run() should warn and return if we can't find section for available resources
        # in that language
        mock_page.return_value.text = (
        ...
        with self.assertLogs(
            ...
            self.write_list.run(
                self.language_info, self.english_info, changes, ChangeLog()
            )
