...
class WriteReport(LanguagePostProcessor):
    ...
    Which worksheet is translated?
    Is the translation 100% complete?
    Is it the same version as the English original?
    ...
    def create_correctbot_mediawiki(self, worksheet: str, language_code: str) -> str:
        ...
        Returns:
            mediawiki string to fill one cell (for one worksheet in the CorrectBot
            column)
        """
        ...
        match = re.match(
            r"^(\d+) corrections, (\d+) suggestions, (\d+) warnings$",
            correctbot_summary,
        )
        if not match:
            # Somehow the edit summary is not as we expect it
            self.logger.warning(
                f"Couldn't parse edit summary '{correctbot_summary}' in page"
                f" CorrectBot:{page}"
            )
            return (
                f'| style="background-color:{Color.RED}" | Invalid. Please run '
                f"CorrectBot again.\n"
            )
        ...
        if int(match.group(3)) > 0:
            ...
            return (
                f'| style="background-color:{Color.RED}" | '
                f"[[CorrectBot:{page}|{match.group(3)} warnings]]\n"
            )
        ...
        # we don't know if still everything is okay or whether there were problems
        # introduced since the
        # last time CorrectBot ran, so we suggest to re-run CorrectBot
        return (
            f'| style="background-color:{Color.ORANGE}" | '
            f'<span title="Possibly outdated: please run CorrectBot again">⚠</span>'
            f" {report_link}\n"
        )
    ...
    def create_mediawiki(
        ...
        content += (
            "Check also the mediawiki [https://www.4training.net/Special:LanguageStats"
        )
        content += (
            f"?language={language_info.language_code}&x=D Language Statistics for"
            f" {language_info.english_name}]"
        )
        return content

    def create_worksheet_overview(
        self, language_info: LanguageInfo, english_info: LanguageInfo
    ) -> str:
        ...
        Args:
            ...
            english_info: LanguageInfo for English - needed because we need English
                            WorksheetInfos
        Returns:
        ...
        content += (
            f"! [[{language_info.english_name}#Available_training_resources_in_"
            f"{language_info.english_name}|"
        )
        content += (
            'Listed?]]\n! Worksheet\n! Translation\n! Progress\n! colspan="2" | '
        ...
        for page, en_worksheet in english_info.worksheets.items():
            lang_worksheet = (
                language_info.worksheets[page]
                if page in language_info.worksheets
                else None
            )
            content += self.create_worksheet_line(
                language_info.language_code, en_worksheet, lang_worksheet
            )
        content += "|}\n"
        return content
    ...
    def create_worksheet_line(
        self,
        language_code: str,
        en_worksheet: WorksheetInfo,
        lang_worksheet: Optional[WorksheetInfo],
    ) -> str:
        """Create mediawiki code with the report for one worksheet
        (one line of the overview)

        Args:
            language_code: Which language we're writing this report line for
                            (we can't use worksheet_info.language_code because
                            worksheet_info may be None)
            en_worksheet: WorksheetInfo for the English original
            lang_worksheet: WorksheetInfo for the translation if it exists,
                            otherwise None
        Returns:
        ...
        if lang_worksheet is not None:
            content += (
                f"| [[{en_worksheet.title}/{language_code}|{lang_worksheet.title}"
                f"]]{self._note(en_worksheet)}\n"
            )
        else:
            content += "| -\n"

        # column 8: Version information (we need to process this here because
        # version_color is needed for other columns)
        ...
        if lang_worksheet is None:
            ...
            version_content = (
                f'| style="background-color:{Color.GREEN}" | {en_worksheet.version}\n'
            )
        else:
            version_content = f'| style="background-color:{Color.RED}" '
            version_content += (
                f"| {lang_worksheet.version} (Original: {en_worksheet.version})\n"
            )

        # column 4: Translation progress
        translated_unit_count: int = (
            lang_worksheet.progress.translated if lang_worksheet is not None else 0
        )
        ...
        if lang_worksheet is None:
            ...
            # This produces a warning in the line for this language in WriteSummary,
            # so make it red
            progress_color = Color.RED
        else:
        ...
        color_css = (
            f";background-color:{progress_color}" if lang_worksheet is not None else ""
        )
        content += f'| style="text-align:right{color_css}" '
        # Add a link to translation view showing either untranslated units
        # (progress < 100%) or translated units
        content += (
            f"| [{self.fortraininglib.index_url}?title=Special:Translate&group=page-"
            f"{en_worksheet.page}"
        )
        ...
        if (
            lang_worksheet is not None
            and (file_info := lang_worksheet.get_file_type_info("pdf")) is not None
        ):
            ...
            if file_info.metadata is not None and not file_info.metadata.correct:
                ...
                if file_info.metadata.version != lang_worksheet.version:
                    ...
                    self.logger.warning(
                        f"{lang_worksheet.page}/{lang_worksheet.language_code} "
                        f"has version "
                        f"{lang_worksheet.version} but PDF has version "
                        f"{file_info.metadata.version}!"
                    )
                    pdf_color = Color.RED
            ...
            if file_info.metadata is not None:
                content += (
                    f'| style="background-color:{pdf_color}" | '
                    f"{file_info.metadata.to_html()}\n"
                )
            else:
            ...
            pdf_color = Color.RED
            content += (
                f'| colspan="2" style="background-color:{Color.RED}; '
                f'text-align:center" | -\n'
            )

        # column 7: Link to translated ODT/ODG file (if existing)
        if lang_worksheet is not None and (
            lang_worksheet.has_file_type("odt") or lang_worksheet.has_file_type("odg")
        ):
            ...
            od_color = Color.RED
            content += (
                f'| style="background-color:{Color.RED}; text-align:center" | -\n'
            )
        ...
        if lang_worksheet is not None:
            content += self.create_correctbot_mediawiki(
                lang_worksheet.page, lang_worksheet.language_code
            )
        else:
            content += "| -\n"
        ...
        if (
            version_color == Color.GREEN
            or progress_color != Color.RED
            or od_color != Color.RED
            or pdf_color != Color.RED
        ):
            line_color = Color.ORANGE
        if (
            version_color == Color.GREEN
            and progress_color == Color.GREEN
            and od_color == Color.GREEN
            and pdf_color == Color.GREEN
        ):
