...
class TranslationSnippet:
    ...
    def is_br(self) -> bool:
        return (self.type == SnippetType.MARKUP_SNIPPET) and bool(
            re.match("<br ?/?>\n?", self.content)
        )
...
class TranslationUnit:
    ...
    def __init__(
        ...
        self._iterate_pos: int = (
            0  # For iterating over all snippets in this TranslationUnit
        )
    ...
    def sync_from_snippets(self):
        ...
        if self._definition_snippets is None or self._translation_snippets is None:
            self.logger.warning(
                "Attempting to sync from non-existing snippets. Ignoring."
            )
            return
    ...
    def get_translation_diff(self) -> str:
        ...
        if self.has_translation_changes():
            seq_mat = difflib.SequenceMatcher(
                a=self._original_translation, b=self._translation
            )
            for operation, a_start, a_end, b_start, b_end in seq_mat.get_opcodes():
                if operation == "delete":
                    diff += (
                        self.RED
                        + "{"
                        + self._original_translation[a_start:a_end]
                        + "}"
                        + self.NO_COLOR
                    )
                elif operation == "replace":
                    diff += (
                        self.RED + "{" + self._original_translation[a_start:a_end] + ","
                    )
                    diff += (
                        self.GREEN
                        + self._translation[b_start:b_end]
                        + "}"
                        + self.NO_COLOR
                    )
    ...
    def remove_links(self):
        ...
        if match_d:
            self.logger.warning(
                f"Found errorneous link {match_d.group(0)} in English original in {self.get_name()}. "
        ...
        if match_t:
            self.logger.warning(
                f"The following link is errorneous: {match_t.group(0)}. "
    ...
    def split_into_snippets(
        text: str, split_all_tags: bool = False
    ) -> List[TranslationSnippet]:
        """
        ...
        for match in re.finditer(pattern, text):
            if (
                (match.group()[0] == "#")
                and (match.start() >= 2)
                and (text[match.start() - 2 : match.start()] == "[[")
            ):
                continue  # Ignore '#' if it's part of an [[#internal link]]
            if match.start() > last_pos:
                text_snippet = TranslationSnippet(
                    SnippetType.TEXT_SNIPPET, text[last_pos : match.start()]
                )
                snippets.append(text_snippet)
            markup_snippet = TranslationSnippet(
                SnippetType.MARKUP_SNIPPET, match.group()
            )
        ...
        if last_pos < len(text):
            snippets.append(
                TranslationSnippet(SnippetType.TEXT_SNIPPET, text[last_pos:])
            )
        return snippets
    ...
    def _ensure_split(self):
        ...
        if self._definition_snippets is None:
            self._definition_snippets = self.split_into_snippets(
                self._definition, self.split_all_tags
            )
        if self._translation_snippets is None:
            self._translation_snippets = self.split_into_snippets(
                self._translation, self.split_all_tags
            )
    ...
    def is_translation_well_structured(self) -> Tuple[bool, str]:
        ...
        self._ensure_split()
        assert (
            self._definition_snippets is not None
            and self._translation_snippets is not None
        )
        ...
        if len(self._definition_snippets) != len(self._translation_snippets):
            ...
            for d_snippet, t_snippet in zip(
                self._definition_snippets, self._translation_snippets
            ):
        ...
        if not is_well_structured:
            ...
            br_in_translation = len(
                [s for s in self._translation_snippets if s.is_br()]
            )
    ...
    def __next__(self):
        ...
        while self._iterate_pos < len(self._definition_snippets):
            if self._iterate_pos >= len(self._translation_snippets):
                self.logger.warning(
                    f"Internal error while iterating over {self.get_name()}: "
    ...
    def __str__(self) -> str:
        ...
        content += (
            f"Definition={self._definition} (original: {self._original_definition}) "
        )
    ...
    def __copy__(self):
        ...
        return TranslationUnit(
            self.identifier, self.language_code, self._definition, self._translation
        )
        ...
        for counter, snippet in enumerate(self._definition_snippets):
            ...
            for other_snippet, other_snippet_translation in other:
                if snippet.content == other_snippet.content:
                    if (
                        self._translation_snippets[counter].content
                        != other_snippet_translation.content
                    ):
                        self.logger.warning(
                            f"{self.get_name()} and {other.get_name()} have same definition: "
...
class TranslatedPage:
    ...
    def __init__(self, page: str, language_code: str, units: List[TranslationUnit]):
        ...
        self._english_info: Optional[WorksheetInfo] = (
            None  # WorksheetInfo of English original
        )
        self._worksheet_info: Optional[WorksheetInfo] = (
            None  # WorksheetInfo of translation
        )
        self._iterate_pos: int = (
            0  # For iterating over all translation units in this TranslatedPage
        )
    ...
    def _analyze_units(self):
        ...
        self._english_info = WorksheetInfo(
            self.page,
            "en",
            headline_original,
            TranslationProgress(len(self.units), 0, len(self.units)),
            version_original,
        )  # noqa: E128
        if odt_original != "":
            self._english_info.add_file_info(
                FileInfo("odt", odt_original, datetime(1970, 1, 1))
            )
        self._worksheet_info = WorksheetInfo(
            self.page,
            self.language_code,
            headline_translation,
            TranslationProgress(translated_units, 0, len(self.units)),
            version_translation,
        )  # noqa: E128
        if odt_translation != "":
            self._worksheet_info.add_file_info(
                FileInfo("odt", odt_translation, datetime(1970, 1, 1))
            )
