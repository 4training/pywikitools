...
https://www.4training.net/api.php?action=query&titles=Start/fa&prop=info
...
if __name__ == "__main__":
    ...
    language_list = json.loads(
        fortraininglib.get_page_source("4training:languages.json")
    )
    counter: int = 0
    warning: int = 0
    for lang in language_list:
        counter += 1
        json = fortraininglib._get(
            {
                "action": "query",
                "titles": f"Start/{lang}",
                "prop": "info",
        ...
        if direction_api != direction_lib:
            warning += 1
            print(
                f"WARNING: fortraininglib.get_language_direction({lang}) returns incorrect result."
            )
