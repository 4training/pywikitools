...
class UniversalCorrector(ABC):
    ...
    def _get_language_code(self) -> str:
        ...
        logger.warning(
            f"Unexpected module name {self.__module__} in _get_language_code()"
        )
        return ""
        ...
        def is_exception(punctuation_pos: int):
            ...
            for exception in self._capitalization_exceptions():
                ...
                for punctuation_match in re.finditer(re.compile(r"[.!?]"), exception):
                    ...
                    if (
                        start_pos >= 0
                        and len(text) > end_pos
                        and text[start_pos:end_pos] == exception
                    ):
                        return True
            return False
    ...
    def correct_missing_spaces(self, text: str) -> str:
        ...
        def does_match_exception(match: Match) -> bool:
            ...
            for exception in self._missing_spaces_exceptions():
                ...
                if not exception_match:
                    logger = logging.getLogger(__name__)
                    logger.warning(
                        f"Ignoring wrong exception '{exception}' for correct_missing_spaces()"
                    )
                    return False
        ...
        while match := check_missing_spaces.search(text, last_start_pos):
            if (
                match.start(1) == 0
            ):  # it would be strange if our text directly started with a punctuation mark.
                last_start_pos += (
                    1  # also the following if statement would raise an IndexError
                )
                continue
            ...
            if does_match_exception(match):
                continue
            text = (
                text[: match.end(1)] + " " + text[match.start(2) :]
            )  # match.expand(r"\1 \2")
    ...
    def correct_mediawiki_bold_italic(self, text: str) -> str:
        ...
        if (
            len(splitted_text) % 2
        ) != 1:  # Not an even amount of ''''': we don't do anything
            logger.warning(
                f"Found uneven amount of bold italic formatting (''''') Please correct manually: {text}"
            )
            return text
        else:
        ...
        if (
            ...
            logger.warning(
                f"Found uneven amount of bold formatting (''') Please correct manually: {text}"
            )
            return text
        else:
        ...
        if (
            ...
            logger.warning(
                f"Found uneven amount of italic formatting ('') Please correct manually: {text}"
            )
            return text
        else:
        ...
        if b_start != b_end:
            logger.warning(
                f"Formatting not even: {b_start} occurrences of <b> but {b_end} occurrences of </b>"
            )
        if i_start != i_end:
            logger.warning(
                f"Formatting not even: {i_start} occurrences of <i> but {i_end} occurrences of </i>"
            )
    ...
    def correct_links(self, text: str, original: str) -> str:
        ...
        ):  # This can happen in testing context
            return text
        logger = logging.getLogger(__name__)
        unchanged_text = text
        ...
        for match_original in re.finditer(find_link, original):
            ...
            if match_translation is None:
                logger.warning(
                    f"Missing [[Destination/{self._get_language_code()}|Link description]] in translation."
                )
            ...
            if len(link_parts) != 2:
                logger.warning(
                    f"Found misformatted link in English original: {match_original.group(0)}. "
                    "Please inform an administrator."
                )

            translated_description = match_translation.group(1).split("|")[-1].strip()
            new_result = (
                text[: match_translation.start()]
                + "[["
                + destination
                + "/"
                + self._get_language_code()
...
class QuotationMarkCorrector(ABC):
    ...
    def _correct_quotes(
        self, start_quotation_mark: str, end_quotation_mark: str, text: str
    ) -> str:
        ...
        if (
            len(splitted_text) % 2
        ) != 1:  # Not an even amount of quotes: we don't do anything
            logger = logging.getLogger(__name__)
            logger.warning(
                f'Found uneven amount of quotation marks (")! Please correct manually: {text}'
            )
        else:
