...
class LibreOffice:
    ...
    def __init__(self, headless: bool = False):
        ...
        self._desktop: Optional[Any] = (
            None  # LibreOffice central desktop element (com.sun.star.frame.Desktop)
        )
    ...
    def open_file(self, file_name: str):
        ...
        while not search_ready and retries < self.CONNECT_TRIES:
            if ctx is None:
                try:
                    ...
                    self.logger.info(
                        f"Failed to connect to LibreOffice: {error}. Retrying..."
                    )
            else:
                ...
                if self._model:
                    try:
                        ...
                        self.logger.info(
                            f"Error while preparing LibreOffice for searching: {error}. Retrying..."
                        )
                    else:
                        search_ready = True
        ...
        if not search_ready:
            raise ConnectionError(
                f"Error trying to access the LibreOffice document {file_name}."
    ...
    def search_and_replace(
        ...
        if found is not None:
            if warn_if_pages_change:
                ...
                self.logger.debug(
                    f"Found {search} on page(s) {start_page_before} to {end_page_before}"
                )
            ...
            found.setString(
                replace if next_match is None else replace[: next_match.start()]
            )
            ...
            if warn_if_pages_change:
                ...
                if (start_page_before != start_page_after) or (
                    ...
                    self.logger.warning(
                        f"Page structure changed while replacing '{search}' with '{replace}'. "
    ...
    def close(self):
        ...
        try:
            ...
            self.logger.error(
                f"soffice process didn't terminate within {self.TIMEOUT}s. Killing it."
            )
            self._proc.kill()
    ...
    def set_default_styles(self, language_code: str, rtl: bool = False):
        ...
        if rtl:
            ...
            default_paragraph_style.ParaAdjust = (
                1  # alignment (0: left; 1: right; 2: justified; 3: center)
            )
            default_paragraph_style.WritingMode = (
                1  # direction (0: LTR; 1: RTL; 4: use superordinate object settings)
            )
        ...
        if language_code in LANG_LOCALE:
            ...
            if lang.has_custom_font():
                self.logger.warning(
                    f'Using font "{lang.get_custom_font()}". Please make sure you have it installed.'
                )
